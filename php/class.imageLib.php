...
## --------------------------------------------------------
  ...
  # Date:     27-02-08    
    ...
    #
    {
    if (!$this->testGDInstalled()) { if ($this->debug) {  throw new exception('The GD Library is not installed.'); }else{ die(); }};
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/ 
  ...
  #       forceStretch can be applied to options 1,2,3 and 4  
    ...
    if (!$this->image) { if ($this->debug) {  throw new exception('file ' . $this->getFileName() .' is missing or invalid'); }else{ die(); }};
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/   
  ...
  { 
    ...
    if (!$this->testEXIFInstalled()) { if ($this->debug) {  throw new exception('The EXIF Library is not installed.'); }else{ return array(); }};
    if (!file_exists($this->fileName)) { if ($this->debug) {  throw new exception('Image not found.'); }else{ return array(); }};
    if ($this->fileExtension != '.jpg') { if ($this->debug) {  throw new exception('Metadata not supported for this image type.'); }else{ return array(); }};
...
## -------------------------------------------------------- 
    ...
    if (!file_exists($file) && !$this->checkStringStartsWith('http://', $file)) { if ($this->debug) {  throw new exception('Image '. $file. ' not found.'); }else{ die(); }};
...
## -------------------------------------------------------- 
  ...
  #       third party class to save as bmp.
    ...
    if (!is_resource($this->imageResized)) { if ($this->debug) { throw new exception('saveImage: This is not a resource.'); }else{ die(); }}      
    $fileInfoArray = pathInfo($savePath);
    clearstatcache();
    if (!is_writable($fileInfoArray['dirname'])) {  if ($this->debug) {  throw new exception('The image path is not writable. Please check your permissions.'); }else{ die(); }} 
...
## --------------------------------------------------------
    ...
    #
  {
    ...
    if (!is_resource($this->imageResized)) { if ($this->debug) {  throw new exception('saveImage: This is not a resource.'); }else{ die(); }}  
        ...
        break;
      case 'bmp':
        throw new exception ('bmp file format is not supported.');
        break;
